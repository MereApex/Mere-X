...
async function waitFor(expression, timeout = 12_000) { const started = Date.now(); while (Date.now() - started < timeout) { if (await evaluate(expression)) return; await sleep(120) } throw new Error(`Timed out waiting for ${expression}`) }
...
try {
  ...
  await evaluate(`localStorage.setItem('mere-x-settings-controls', JSON.stringify({...JSON.parse(localStorage.getItem('mere-x-settings-controls') || '{}'), voiceInput:true})); location.hash='/app'; location.reload()`)
...
} catch (error) { console.error(JSON.stringify({ ok: false, error: error.message, runtimeErrors }, null, 2)); process.exitCode = 1 } finally { socket.close() }
